...
# ── Minimal CSS: just enough to feel intentional, nothing fighting Streamlit ─
st.markdown("""
<style>
  ...
  .metric-box {
    ...
    border: 1px solid rgba(120,120,120,0.2); border-radius: 8px;
  }
  .metric-box .value { font-size: 1.4rem; font-weight: 600; }
  .metric-box .label { font-size: 0.75rem; opacity: 0.6; text-transform: uppercase; }
</style>
""", unsafe_allow_html=True)
...
defaults = {
    ...
    "file_count":   0,
    "module_count": 0,
    "stage":        "idle",   # idle | fetching | done | error
    "error_msg":    "",
}
for k, v in defaults.items():
    if k not in st.session_state:
...
st.caption("Point it at any public GitHub repo. Get comprehensive docs in seconds.")
st.divider()
...
with col1:
    title = st.text_input("Project Title", placeholder="My Awesome Project")
with col2:
    description = st.text_input("Short Description", placeholder="What does this project do?")
...
if run:
    if not repo_url.strip():
        ...
        try:
            ...
            with st.spinner("Fetching repo, analyzing code, and generating docs…"):
                graph = build_graph()
                result = graph.invoke({
                    "repo_url":    repo_url,
                    "title":       title,
...
if st.session_state.final_doc:
    ...
    <div class="metric-row">
      <div class="metric-box"><div class="value">{fc}</div><div class="label">Files found</div></div>
      <div class="metric-box"><div class="value">{mc}</div><div class="label">Modules analyzed</div></div>
      <div class="metric-box"><div class="value">{wc:,}</div><div class="label">Words generated</div></div>
    </div>
    """, unsafe_allow_html=True)

    # ── Export ───────────────────────────────────────────────────────────────
    col_md, col_gen, col_dl = st.columns(3)
...
# ── Footer ───────────────────────────────────────────────────────────────────
st.divider()
st.caption("Built with LangGraph · Llama 3.1 · Streamlit")